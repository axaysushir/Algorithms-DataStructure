...
// Until now, we have kept the starting index of subarray fixed, and found the last position. Instead, we could move the starting index of the current subarray as soon as we 
// know that no better could be done with this index as the starting index. We could keep 2 pointer,one for the start and another for the end of the current subarray, and make 
// optimal moves so as to keep the sum\text{sum}sum greater than S as well as maintain the lowest size possible.
//
...
//     While sum\text{sum}sum is greater than or equal to S:
