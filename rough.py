...
class Node:
    ...
    def __repr__(self):
        if self in self.printvisited:
            ...
            return final_str + f'({self.val}, ({[n.val for n in self.adj]}))'

class Solution():
    lookup = {None: None}
    def deepcopy(self, graphNode):
        if graphNode not in self.lookup:
            self.lookup[graphNode] = Node(graphNode.val)
            self.lookup[graphNode] = [self.lookup.setdefault(neighbor, self.deepcopy(neighbor)) for neighbor in graphNode.adj]
        return self.lookup[graphNode]
...
n4.adj = [n3, n2]
n2.adj = [n4]
graphcopy = Solution().deepcopy(n1)
print(graphcopy)